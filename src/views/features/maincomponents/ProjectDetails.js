...
const ProjectDetails = () => {
  ...
  function deleteTarget(urlid) {
        const token = cookie.get('token')
        // Get all details for the project
        axios.delete(`http://127.0.0.1:5000/projects/${projectid}/targets/${urlid}?token=${token}`)
      .then(
        setTargets(targets.filter(
            function(target) { 
    ...
    {loading ? (
      ...
      <div style={{paddingBottom: "30px"}}>
          ...
          <div className='headerright'>
              <CButton onClick={() =>setVisible(true)} color='primary'><b>Create Target</b></CButton>
          </div>
          </div>
        ...
        {targets.map((target) => (
            <div className="individualproject">
              ...
              <div className="floatright">
                <CButton onClick={() => deleteTarget(target.urlid)} shape='pill' variant='outline' color='warning'>
              ...
              <div className="floatright">
                <div className="floatright">
                  <h6 className="projectinfo">
                    {calculateUptime(target.numsuccess, target.numfailure) + "% UPTIME"}
                    <CIcon style={{marginBottom: "4px", marginLeft: "2px"}} content={freeSet.cilChevronDoubleUp}></CIcon>
                  </h6>
                </div>
                <div className="floatright">
                  <CButton
